...
- (void)setMode:(DJIGSViewMode)mode
{
    ...
    [_addPadBtn setHidden:(mode == DJIGSViewMode_ViewMode)];
}
...
- (IBAction)addPadBtnAction:(id)sender {
    if ([_delegate respondsToSelector:@selector(addPadBtnActionInGSButtonVC:)]) {
        [_delegate addPadBtnActionInGSButtonVC:self];
    }
}
